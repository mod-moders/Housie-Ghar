...
// ── Brand wordmark (placeholder, swappable for real logo) ────────────────────
function Logo({ size = 22, onClick }) {
  return (
    <button className="hg-logo" onClick={onClick} aria-label="Housie Ghar home"
      style={{ fontSize: size }}>
      <span className="hg-logo-cage" aria-hidden="true">
        <Icon name="grid" size={size * 0.92} strokeWidth={2.4} />
      </span>
      <span className="hg-logo-text">
        <span className="hg-logo-h">HOUSIE</span>
        <span className="hg-logo-g">GHAR</span>
      </span>
    </button>
  );
}
...
function TrustBadges() {
  return (
    <div className="hg-trust">
      <div className="hg-trust-item">
        <Icon name="shieldCheck" size={20} />
        <div>
          <strong>Provably Fair RNG</strong>
          <span>Cryptographic draws, audited</span>
        </div>
      </div>
      <div className="hg-trust-item">
        <Icon name="wallet" size={20} />
        <div>
          <strong>Zero-Commission P2P</strong>
          <span>Pay your agent directly</span>
        </div>
      </div>
    </div>
  );
    ...
    <header className="hg-nav">
      ...
      <nav className="hg-nav-links">
        {items.map(([key, lbl, icon]) => (
          <button key={lbl} className={`hg-nav-link${current === key && lbl !== 'Games' ? ' is-active' : ''}`}
