              ...
              return (
                ...
                >
                  <div className="hg-fill-top">
                    ...
                    <span className={`hg-pill hg-pill-${g.game_status.toLowerCase().replace("_", "-")}`} style={{ whiteSpace: "nowrap" }}>{g.game_status === "Draw_Ended" ? "Game Ended" : g.game_status.replace("_", " ")}</span>
                  </div>
      ...
      <div className="hg-live-body" style={{ marginTop: "12px" }}>
        <div className="hg-live-left">
          ...
          <div className="hg-cage-area">
            <RealisticBingoCage lastDrawn={lastDrawn ?? null} isTeasing={!revealed && (status === "Live" || status === "Paused")} numberRevealed={numberRevealed} muted={muted} />
...
export function OverflowSection({ me }: { me: AuthUser }) {
  ...
  const sendUpiToPlayer = (r: QueueBooking) => {
    const upiId = r.staff_upi || me.upi_id || "";
    if (!upiId) {
                    ...
                    <div>
                      <div style={{ display: "flex", alignItems: "center", gap: "6px" }}>
                        ...
                        <span style={{ fontSize: "10px", background: "rgba(255,255,255,0.06)", padding: "2px 6px", borderRadius: "4px", color: "var(--text-mute)" }}>
                          {d.formatted_booking_id || `#${d.booking_id.substring(0, 8).toUpperCase()}`}
                        </span>
                      </div>
              ...
              <div key={g.game_id} className={`hg-fill-card${presetClass ? " " + presetClass : ""}`} style={{ display: "flex", flexDirection: "column", minHeight: "128px", position: "relative" }}>
                <div>
                  <div className="hg-fill-top" style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 8 }}>
                    ...
                    <span className={`hg-pill hg-pill-${g.game_status.toLowerCase().replace("_", "-")}`} style={{ flexShrink: 0, whiteSpace: "nowrap" }}>{g.game_status === "Draw_Ended" ? "Game Ended" : g.game_status.replace("_", " ")}</span>
                  </div>
