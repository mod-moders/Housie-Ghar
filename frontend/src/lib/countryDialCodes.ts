/**
 * Country dial codes for the phone entry fields.
 *
 * India is first and is the default selection — this platform's market is India.
 * The remainder are alphabetical by name.
 *
 * Flags are not stored. `flagEmoji()` derives them from the two-letter ISO code
 * using Unicode regional indicator symbols, so there is no image asset, no
 * network request, and no per-row string that can drift out of sync.
 */
